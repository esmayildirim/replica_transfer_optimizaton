...
#MULTI-VARIATE MULTI-INPUT MLP MODEL
...
def MLP_model_train(X_train,y_train):
    model = Sequential()
    ...
    for i in range(NUM_LAYERS-1):
        model.add(Dense(NUM_NODES, activation='relu', kernel_initializer = 'he_normal', bias_initializer = 'he_normal'))#1st hidden layer
    model.add(Dense(y_train.shape[1])) # output layer
    opt = Adam(learning_rate=0.0001)
    model.compile(optimizer=opt, loss='mae', metrics = ['mae'])# mse is changed to mape

    checkpoint = ModelCheckpoint('mlp_best_weights_mv_mi_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_LAYERS)+"_"+str(NUM_NODES)+'.h5',
                             monitor='val_loss',
                             verbose=1,
                             mode='min',
                             save_best_only=True)
    ...
    plt.savefig('epocs_mlp_mv_mi_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_LAYERS)+"_"+str(NUM_NODES)+'.eps', dpi = 300)
    plt.close()
    return model
    
def MLP_model_test(checkpoint_filepath, X_test, y_test):
    ...
    for i in range (X_test.shape[0]):
        ...
        for j in range(y_test.shape[1]//6): # the number of output timestamps
            for k in range(6): # number of dimensions
                ...
                if(y_test_output[0][j*6+k] != 0.0):
                    ...
                    counter+=1
    for j in range(6):
        mae[j] /= (counter*(y_test.shape[1]//6))
        mape[j] /=(counter*(y_test.shape[1]//6))
        mape[j] *= 100
...
if __name__ == '__main__':
    ...
    #TRAINING
    model = MLP_model_train(X_train,y_train)

    #TESTING
    mae_list, mape_list, R2 = MLP_model_test('mlp_best_weights_mv_mi_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_LAYERS)+"_"+str(NUM_NODES)+'.h5', X_test, y_test)
