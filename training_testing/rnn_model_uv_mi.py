...
def RNN_model_train(X_train,y_train, metric_name):
    n_features = X_train.shape[2]
    n_output = y_train.shape[1] * y_train.shape[2]
    y_train = y_train.reshape((y_train.shape[0], n_output))
    model = Sequential()
    if NUM_RNN_LAYERS == 1:
        model.add(SimpleRNN(NUM_NODES, activation='relu', input_shape = (NSTEPS_BACK, n_features), kernel_initializer = 'he_normal'))
    elif NUM_RNN_LAYERS == 2:
        model.add(SimpleRNN(NUM_NODES, activation='relu', input_shape = (NSTEPS_BACK, n_features), kernel_initializer = 'he_normal', return_sequences = 'true'))
        model.add(SimpleRNN(NUM_NODES, activation='relu', kernel_initializer = 'he_normal'))
    else:
        model.add(SimpleRNN(NUM_NODES, activation='relu', input_shape = (NSTEPS_BACK, n_features), kernel_initializer = 'he_normal', return_sequences = 'true'))
        for i in range(NUM_RNN_LAYERS-2):
            model.add(SimpleRNN(NUM_NODES, activation='relu', kernel_initializer = 'he_normal', return_sequences = 'true'))
        model.add(SimpleRNN(NUM_NODES, activation='relu', kernel_initializer = 'he_normal'))
    ...
    checkpoint = ModelCheckpoint('rnn_best_weights_uv_mi_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_RNN_LAYERS)+"_"+str(NUM_NODES)+"_"+metric_name+'.h5',
                             monitor='val_loss',
                             verbose=1,
                             mode='min',
                             save_best_only=True)
   ...
   # plt.savefig('epocs'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_RNN_LAYERS)+"_"+str(NUM_NODES)+"_"+metric_name+'.eps', dpi = 300)
   # plt.close()
    return model
def RNN_model_test_BD(checkpoint_filepath, X_test, y_test):
...
def RNN_model_test_TRL(checkpoint_filepath, X_test, y_test):
...
if __name__ == '__main__':
    ...
    #TRAINING
    model_BD = RNN_model_train(X_train_BD,y_train_BD, "BD")
    model_TRL = RNN_model_train(X_train_TRL,y_train_TRL, "TRL")
    
    #TESTING
    mae_list_BD, mape_list_BD, R2_BD = RNN_model_test_BD('rnn_best_weights_uv_mi_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_RNN_LAYERS)+"_"+str(NUM_NODES)+'_BD.h5', X_test_BD, y_test_BD)
    mae_list_TRL, mape_list_TRL, R2_TRL = RNN_model_test_TRL('rnn_best_weights_uv_mi_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_RNN_LAYERS)+"_"+str(NUM_NODES)+'_TRL.h5', X_test_TRL, y_test_TRL)
