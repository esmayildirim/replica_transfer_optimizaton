...
def LSTM_model_train(X_train,y_train, metric_name):
    ...
    checkpoint = ModelCheckpoint('lstm_best_weights_uv_si_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_LSTM_LAYERS)+"_"+str(NUM_NODES)+"_"+metric_name+'.h5',
                             monitor='val_loss',
                             verbose=1,
                             mode='min',
                             save_best_only=True)
    ...
    #plt.savefig('epocs_lstm_uv_si'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_LSTM_LAYERS)+"_"+str(NUM_NODES)+"_"+metric_name+'.eps', dpi = 300)
    #plt.close()
    return model
...
if __name__ == '__main__':
    ...
    #TESTING
    mae_BD, mape_BD, R2_BD = LSTM_model_test_BD('lstm_best_weights_uv_si_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_LSTM_LAYERS)+"_"+str(NUM_NODES)+'_BD.h5', X_test_BD, y_test_BD)
    mae_TRL, mape_TRL, R2_TRL = LSTM_model_test_TRL('lstm_best_weights_uv_si_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_LSTM_LAYERS)+"_"+str(NUM_NODES)+'_TRL.h5', X_test_TRL, y_test_TRL)
