...
def MLP_model_train(X_train,y_train, var_name):
    ...
    checkpoint = ModelCheckpoint('mlp_best_weights_uv_si_'+var_name+'_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_LAYERS)+"_"+str(NUM_NODES)+'.h5',
                             monitor='val_loss',
                             verbose=0,
                             mode='min',
                             save_best_only=True)
    ...
    plt.savefig('epocs_mlp_uv_si_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_LAYERS)+"_"+str(NUM_NODES)+'.eps', dpi = 300)
    plt.close()
    return model
...
if __name__ == '__main__':
    ...
    mae_BD, mape_BD, R2_BD = MLP_model_test_BD('mlp_best_weights_uv_si_'+"BD"+'_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_LAYERS)+"_"+str(NUM_NODES)+'.h5', X_test_BD, y_test_BD)
    mae_TRL, mape_TRL, R2_TRL = MLP_model_test_TRL('mlp_best_weights_uv_si_'+"TRL"+'_'+str(NSTEPS_BACK)+"_"+str(NSTEPS_FUTURE)+"_layers"+str(NUM_LAYERS)+"_"+str(NUM_NODES)+'.h5', X_test_TRL, y_test_TRL)
